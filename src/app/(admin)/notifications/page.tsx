...
export default function NotificationsPage() {
  ...
  return (
    <div className="p-8 max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-3 bg-white mboka-card border rounded-xl px-4 py-3 shadow-sm">
      ...
      <div className="space-y-2">
        {MOCK_NOTIFS.map(n => {
          ...
          return (
            <div
              key={n.id}
              className={`flex items-start gap-4 bg-white mboka-card border rounded-xl px-4 py-4 shadow-sm transition-opacity ${n.read ? 'opacity-60' : ''}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className={`text-sm font-medium ${n.read ? 'text-[#6B6560]' : 'text-[#1a1a1a]'}`}>{n.title}</p>
                  {!n.read && (
                    <span className="w-1.5 h-1.5 rounded-full bg-[#C8A96E] shrink-0" />
                  )}
                </div>
                <p className="text-xs text-[#7A726B] mt-0.5">{n.body}</p>
                <p className="text-[11px] text-[#B0A9A2] mt-1">{n.time}</p>
              </div>
            </div>
          )
        })}
      ...
      <div className="flex flex-col items-center gap-3 py-6 text-center">
        <div className="w-12 h-12 rounded-full bg-[#F0EBE2] border border-[rgba(0,0,0,0.06)] flex items-center justify-center">
          <BellRing size={20} className="text-[#B0A9A2]" />
        </div>
        <div>
          <p className="text-sm text-[#9B928B]">Bientôt : notifications push et emails automatiques</p>
          <p className="text-xs text-[#B0A9A2] mt-0.5">Cotisations impayées · Nouveaux membres · Rappels d'événements</p>
        </div>
      </div>
    </div>
  )
