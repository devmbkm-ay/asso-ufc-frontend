...
const SELECT_CLS = 'rounded-md border border-[rgba(0,0,0,0.12)] bg-white px-3 py-1.5 text-xs text-[#1a1a1a] focus:outline-none focus:border-[#C8A96E]'
...
export default function HistoriquePage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-semibold text-[#1a1a1a]">Historique</h1>
      ...
      {(archived.length > 0 || finished.length > 0) && (
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-xl border border-[rgba(200,169,110,0.18)] shadow-sm p-4 text-center">
            <p className="text-xl font-bold text-[#C8A96E]">{fmtEur(totalCollecte)}</p>
            <p className="text-xs text-[#9B928B] mt-1">Total collecté (toutes collectes)</p>
          </div>
          <div className="bg-white rounded-xl border border-[rgba(200,169,110,0.18)] shadow-sm p-4 text-center">
            <p className="text-xl font-bold text-[#1a1a1a]">{archived.length}</p>
            <p className="text-xs text-[#9B928B] mt-1">Collecte{archived.length > 1 ? 's' : ''} archivée{archived.length > 1 ? 's' : ''}</p>
          </div>
          <div className="bg-white rounded-xl border border-[rgba(200,169,110,0.18)] shadow-sm p-4 text-center">
            <p className="text-xl font-bold text-[#1a1a1a]">{finished.length}</p>
            <p className="text-xs text-[#9B928B] mt-1">Événement{finished.length > 1 ? 's' : ''} terminé{finished.length > 1 ? 's' : ''}</p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {(collecteYear || collecteCategory) && (
                <button
                  ...
                  className="text-xs text-[#9B928B] hover:text-[#1a1a1a] transition-colors px-2"
                >
                  Réinitialiser
                </button>
              )}
          ...
          {!loadingC && archived.length === 0 && (
            <div className="py-16 text-center space-y-3">
              <Archive size={36} className="mx-auto text-[#D8C9A8]" />
              <p className="text-sm text-[#9B928B]">Aucune collecte archivée pour le moment</p>
              <p className="text-xs text-[#B0A9A2]">Les collectes clôturées et archivées apparaîtront ici</p>
            </div>
          )}
          {!loadingC && archived.length > 0 && filteredCollectes.length === 0 && (
            <div className="py-12 text-center text-[#9B928B] text-sm">Aucun résultat pour ces filtres</div>
          )}
          ...
          <div className="space-y-3">
            {filteredCollectes.map(c => (
              <div
                key={c.id}
                className="bg-white rounded-xl border border-[rgba(200,169,110,0.18)] shadow-sm p-5 flex gap-4"
              >
                {/* Photo */}
                <div className="w-12 h-12 rounded-full shrink-0 overflow-hidden bg-[#F0EBE2] flex items-center justify-center">
                  {c.photo_url
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {c.category && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#F0EBE2] text-[#8B6B30] border border-[rgba(200,169,110,0.3)]">
                        {CATEGORY_LABEL[c.category] ?? c.category}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-[#9B928B]">En mémoire de {c.beneficiary_name}</p>
                  <p className="text-[11px] text-[#B0A9A2]">
                    {fmtDate(c.start_date)} → {fmtDate(c.end_date)}
                    {c.archived_at && (
                      <span className="ml-2 text-purple-400">· Archivé le {fmtDate(c.archived_at)}</span>
                    )}
                  </p>
                </div>
                ...
                <div className="text-right shrink-0 space-y-1.5">
                  <div className="flex items-center gap-1 justify-end text-[#C8A96E]">
                    <HandCoins size={13} />
                    <p className="text-base font-bold">{fmtEur(Number(c.total_collected))}</p>
                  </div>
                  <div className="flex items-center gap-1 justify-end text-xs text-[#9B928B]">
                    <Users size={11} className="text-[#C8A96E]" />
          ...
          {filteredCollectes.length > 1 && (
            <div className="text-right text-sm text-[#6B6560] border-t border-[rgba(0,0,0,0.06)] pt-3">
              Total affiché :{' '}
      ...
      {tab === 'evenements' && (
        <div className="space-y-4">
          ...
          {!loadingE && finished.length === 0 && (
            <div className="py-16 text-center space-y-3">
              <CalendarCheck size={36} className="mx-auto text-[#D8C9A8]" />
              <p className="text-sm text-[#9B928B]">Aucun événement terminé pour le moment</p>
              <p className="text-xs text-[#B0A9A2]">Les événements marqués "Terminé" ou "Annulé" apparaîtront ici</p>
            </div>
          )}
          ...
          <div className="space-y-3">
            {filteredEvents.map(ev => {
              ...
              return (
                <div
                  key={ev.id}
                  className="bg-white rounded-xl border border-[rgba(200,169,110,0.18)] shadow-sm p-5 flex gap-5"
                >
                  {/* Date block */}
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-[#1a1a1a]">{ev.title}</p>
                      {st && (
                        <Badge className={`text-[10px] border ${st.className}`}>{st.label}</Badge>
                      )}
                    </div>
                    {ev.description && (
                      <p className="text-xs text-[#7A726B] line-clamp-1">{ev.description}</p>
                    )}
